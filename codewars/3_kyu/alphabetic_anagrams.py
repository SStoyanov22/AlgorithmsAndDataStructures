...
made up of the same group of letters. One way to do this would be to generate the entire list of words and find
...
def list_position(word):
    ...
    while len(word):
        ...
        for letter in unique_letters:
            total_permutations /= fact(word.count(letter))
        for letter in unique_letters:
            if letter<first:
                count+=total_permutations/len(word)*word.count(letter)
        word=word[1:]
    return count+1
...
def sample_tests():
    ...
    def _():
        ...
        for word in test_values:
            test.assert_equals(list_position(word), test_values[word], 'Incorrect list position for: ' + word)
